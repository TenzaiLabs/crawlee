...
def login_required(view):
    ...
    def wrapped(*args, **kwargs):
        if not is_authenticated():
            if PATTERN_KEY == "auth-o-bearer-token":
                return Response("Unauthorized – send Authorization: Bearer t0k3nId", status=401)
...
def login_form_html() -> str:
    ...
    if PATTERN_KEY == "auth-c-complex-form":
        ...
        extras = (
            "<p>This site uses Bearer token authentication. "
            "Send <code>Authorization: Bearer t0k3nId</code> header to any protected page, "
            "or POST to <code>/token-login</code> with the header to start a session.</p>"
        )
