...
def test_full_testsite_exclusion_positive_control() -> None:
    ...
    with ExitStack() as stack:
        ...
        for case in cases:
            guarded_paths = _katana_paths(
                case.target_url,
                headers=case.headers,
                exclusions=list(crawler.DEFAULT_EXCLUSION_PATTERNS),
            )
            assert case.expected_path in guarded_paths, (case.name, sorted(guarded_paths))
            assert guarded_paths.isdisjoint(case.blocked_paths), (
                case.name,
                sorted(guarded_paths),
            )

            unguarded_paths = _katana_paths(
                case.target_url,
                headers=case.headers,
                exclusions=[],
            )
            assert case.blocked_paths <= unguarded_paths, (
                case.name,
                sorted(unguarded_paths),
            )
