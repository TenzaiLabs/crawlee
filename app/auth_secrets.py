...
_TOTP_SEED_TEMPLATE = re.compile(r"\{\{totp_seed:([A-Za-z2-7]+=*)\}\}", re.IGNORECASE)


def resolve_secrets(auth_config: dict) -> dict:
    """Replace {{env:VAR}}, {{totp:VAR}}, and {{totp_seed:SECRET}} in strings.
    ...
    def _resolve_string(value: str) -> str:
        ...
        def _totp_repl(match: re.Match[str]) -> str:
            ...
            return _totp_from_secret(os.environ[var])

        def _totp_seed_repl(match: re.Match[str]) -> str:
            return _totp_from_secret(match.group(1))

        resolved = _ENV_TEMPLATE.sub(_env_repl, value)
        resolved = _TOTP_TEMPLATE.sub(_totp_repl, resolved)
        resolved = _TOTP_SEED_TEMPLATE.sub(_totp_seed_repl, resolved)
        return resolved
